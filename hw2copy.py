...
def scoreList(Rack):
    ...
    return map(lambda x:[x,wordScore(x,scrabbleScores)],removeAll('',map(lambda y:y if wordPossible(Rack,y)else '',Dictionary)))

def bestWord(Rack):
    '''Returns the best word and its score that can be made from the given rack'''
    scores = scoreList(Rack)
    if scores == []: return ['',0]
    else: return highScore(scores[0],scores[1:])
    
def highScore(first, words):
    '''Returns the highest scoring word score tuple from the list'''
    if words==[]: return first
    elif first[1]<words[0][1]: return highScore(words[0],words[1:])
    else: return highScore(first, words[1:])
    
print(scoreList(['a','m','b','r','t','s']))
print(bestWord(['a','m','b','r','t','s']))